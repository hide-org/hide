...
func init() {
	...
	pf.StringVar(&workspace, "workspace", "/workspace", "path to the workspace directory")
...
var serverRunCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		lspService := lsp.NewService(languageDetector, lsp.LspServerExecutables, diagnosticsStore, clientPool, fmt.Sprintf("file://%s", workspace))
		...
		listener, err := net.Listen("unix", "/tmp/hide/hide.sock")
		if err != nil {
		...
		if err := http.Serve(listener, router); !errors.Is(err, http.ErrServerClosed) {
